...
function openModal(src) {
  ...
  content.innerHTML = '';
  const isVideo = src.toLowerCase().match(/\.(mp4|webm|mov|ogg)$/);

  // 定义统一的缩放样式：
  // max-width: 90vw (不超过屏幕宽度的90%)
  // max-height: 85vh (不超过屏幕高度的85%，留出上下间距)
  // object-fit: contain (保持比例缩放)
  const mediaStyle = `max-width: 90vw; max-height: 85vh; object-fit: contain; border: 3px solid white; border-radius: 8px; box-shadow: 0 0 20px rgba(0,0,0,0.5); display: block; margin: auto;`;

  if (isVideo) {
    content.innerHTML = `<video src="${src}" controls autoplay loop style="${mediaStyle}"></video>`;
  } else {
    content.innerHTML = `<img src="${src}" style="${mediaStyle}">`;
  }
